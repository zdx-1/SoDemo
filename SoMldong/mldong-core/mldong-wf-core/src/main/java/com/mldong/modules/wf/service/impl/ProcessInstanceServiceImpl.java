...
public class ProcessInstanceServiceImpl extends ServiceImpl<ProcessInstanceMapper, ProcessInstance> implements ProcessInstanceService {
    ...
    public HighLightVO highLight(Long processInstanceId) {
        ...
        if(processInstance!=null) {
            ...
            // 拿到历史任务-按更新时间倒序
            List<ProcessTask> hisProcessTaskList = flowEngine.processTaskService().list(
                    Wrappers.lambdaQuery(ProcessTask.class)
                    .eq(ProcessTask::getProcessInstanceId,processInstanceId)
                    .orderByDesc(ProcessTask::getUpdateTime)
            );
     ...
     * @param processTaskList 正在进行的任务
     * @param hisProcessTaskList 正在进行的任务
     * @param taskName     当前任务节点名称（递归停止标识）
     * @param vo           结果
     */
    private void recursionModel(NodeModel nodeModel, ProcessInstance processInstance, List<ProcessTask> processTaskList,List<ProcessTask> hisProcessTaskList, String taskName, HighLightVO vo) {
        ...
        if (!vo.getHistoryNodeNames().contains(nodeModel.getName())) {
            ...
            nodeModel.getOutputs().stream().filter(output -> {
                ...
                if (nodeModel instanceof DecisionModel) {
                    ...
                    historyTask = hisProcessTaskList.stream().filter(hisTask -> {
                ...
                if (nodeModel instanceof DecisionModel) {
                    ...
                    String handleClass = ((DecisionModel) nodeModel).getHandleClass();
                    if (StrUtil.isNotEmpty(expr)) {
                        String nextNodeName = Convert.toStr(ExpressionUtil.eval(expr,args));
                        ...
                        execution.setNodeModel(nodeModel);
                        execution.setArgs(BeanUtil.toBean(args,Dict.class));
                        String nextNodeName = decisionHandler.decide(execution);
                        return output.getTo().equals(nextNodeName);
                    } else {
                        return false;
                    }
                ...
                if (!vo.getHistoryEdgeNames().contains(transitionModel.getName())) {
                    ...
                    recursionModel(transitionModel.getTarget(), processInstance, processTaskList,hisProcessTaskList, taskName, vo);
                }
            });
        }
    }
