...
<script setup>
  ...
  import { sysDictSaveOrUpdate } from '/@/api/sys/dict';
  ...
  const schemas = [
    ...
    },
    {
      ...
      label: '数据类型',
      component: 'ApiDict',
      helpMessage: ['和使用该字典的字段数据类型相关'],
      rules: [{ required: true, message: '请选择数据类型', trigger: 'blur', type: 'number' }],
      colProps: {
        xl: 24,
        xxl: 24,
      },
  ...
  const handleOk = () => {
    ...
    validate()
      .then((values) => {
        ...
        const successMsg = `${isUpdate.value ? '编辑' : '新增'}字典成功`;
        // const failMsg = `${isUpdate.value ? '编辑' : '新增'}字典失败`;
        // 调用添加或编辑接口
        sysDictSaveOrUpdate(formData, unref(isUpdate))
          .then(() => {
